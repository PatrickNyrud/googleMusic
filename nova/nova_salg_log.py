...
class salg_log:
    def __init__(self, frame):
        self.frame = frame
        self.scrollbar_canvas = tk.Canvas(self.frame, bg = "grey17", width = 1700)
        self.main_frame = tk.Frame(self.frame, width = 1000, bg = "grey21", height = self.file_len() * 40, highlightthickness = 5, highlightbackground = "gray6")
    ...
    def display_log(self):
        for j, line in enumerate(reversed(open("logs//salgs_log.txt").readlines())):
            self.tmp_var = ""
            for x in line.split(","):
                self.tmp_var += x.strip() + "\n"
            self.log_label = tk.Label(self.main_frame, text = self.tmp_var, font = self.text_font, bg = "grey21", fg = "cornsilk2")
